...
param(
    ...
    [string]$Output = "data\training\rfdetr-small-fire-smoke-v1",
...
$stdoutPath = Join-Path $LogRoot "fireviewer-rfdetr-small-v1.out.log"
$stderrPath = Join-Path $LogRoot "fireviewer-rfdetr-small-v1.err.log"
...
if ([string]::IsNullOrWhiteSpace($DatasetRoot) -or -not (Test-Path -LiteralPath $DatasetRoot)) {
    throw "Set FIREVIEWER_DETECTION_DATASET_ROOT to the prepared detection corpus."
}
